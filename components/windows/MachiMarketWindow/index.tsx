...
const MachiMarketWindow = ({ theme }: WindowProps) => {
  ...
  const shopButton = (url: string) => (
    <Button themeName='light' icon={<ExternalLinkIcon height={16} />} href={url}>
      shop
    </Button>
  );
  const readBlogPostButton = (
    <Button themeName='light' variant='secondary' icon={<ExternalLinkIcon height={16} color={theme.textSecondary} />} href='https://mirror.xyz/0x57D1eAE9f0972723F0e78EAF4e6C08e90565206F/AtKMzoVY1qzrvA1gal1XqJv74bpx-sg9ptv5TDw2maU'>
      read blog post
    </Button>
  );
  ...
  const currentContentMetadata: ContentPaneMetadataType = {
    ...
    caption: <Caption theme={theme} caption={machiMarketItems[currentImageIndex].caption} />,
    buttons: [shopButton(`https://opensea.io/assets/optimism/${currentImageIndex}`)]
  };
  ...
  return (
    <div className={styles.container}>
      <CollectionModal theme={theme} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} image={currentImage} content={currentContentPane} onNext={handleNextClicked} onPrev={handlePreviousClicked} showNextButton={showNextButton} showPrevButton={showPreviousButton} />
